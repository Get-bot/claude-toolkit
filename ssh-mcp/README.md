...
- 터미널 직접 접속 CLI — `ssh-mcp connect <alias>`로 등록된 호스트에 터미널에서 바로 SSH 접속하고, `ssh-mcp exec <alias> -- <명령>`으로 명령 하나를 실행한다. `hosts.json`의 주소·포트·사용자명·개인키·호스트 키 지문을 그대로 쓰므로 `host add` 한 번으로 Claude와 사람이 같은 접속 정보를 공유한다. MCP 서버와는 별개의 편의 기능이며, 사람이 직접 치는 경로이므로 분류·승인·감사 게이트를 거치지 않는다. 시스템 `ssh` 클라이언트에 위임하면 MCP 도구가 거부하는 대화형 프로그램(vim, top)도 이 경로에서는 그대로 쓸 수 있다. 인자 없는 `ssh-mcp <alias>` 형태는 두지 않는다 — 인자 없는 실행은 서버 모드이고, alias가 `doctor`·`help` 같은 명령 이름과 겹칠 수 있기 때문이다. 이름은 다른 명령처럼 줄임말 없이 쓴다(`conn`이 아니라 `connect`). 구현 계획은 아직 없다.
- 터미널 직접 접속 CLI — `ssh-mcp connect <alias>`로 등록된 호스트에 터미널에서 바로 SSH 접속하고, `ssh-mcp exec <alias> -- <명령>`으로 명령 하나를 실행한다. `hosts.json`의 주소·포트·사용자명·개인키·호스트 키 지문을 그대로 쓰므로 `host add` 한 번으로 Claude와 사람이 같은 접속 정보를 공유한다. MCP 서버와는 별개의 편의 기능이며, 사람이 직접 치는 경로이므로 분류·승인·감사 게이트를 거치지 않는다. 시스템 `ssh` 클라이언트에 위임하면 MCP 도구가 거부하는 대화형 프로그램(vim, top)도 이 경로에서는 그대로 쓸 수 있다. 인자 없는 `ssh-mcp <alias>` 형태는 두지 않는다 — 인자 없는 실행은 서버 모드이고, alias가 `doctor`·`help` 같은 명령 이름과 겹칠 수 있기 때문이다. 이름은 다른 명령처럼 줄임말 없이 쓴다(`conn`이 아니라 `connect`). 구현 계획은 아직 없다.
