...
| File                  | Description                                                                                                            |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `host-integration.md` | AC3~AC6 수동 호스트 통합 체크리스트 + 릴리스 전 필수 실호스트 확인 절차. `.omc/plans/ssh-mcp-plan.md` §8.5를 따릅니다. |
...
- **AC5** — Claude Code 등록(`claude mcp add ssh-mcp -- cmd /c npx -y @get-bot/ssh-mcp`)과 `/mcp`에서의 도구 목록.
- **AC6** — Windows 11 검증.
...
- 도구 개수(7), 등록 스니펫, 환경변수 이름을 바꿨다면 이 문서와 `README.md`, `src/doctor/checks.ts`의 `buildSnippets()`를 함께 갱신해야 합니다.

## Dependencies

### Internal

없음 (문서 전용). 내용상 `README.md`, `src/doctor/checks.ts`의 등록 스니펫, `tests/e2e/realHost.test.ts`와 동기화돼야 합니다.
