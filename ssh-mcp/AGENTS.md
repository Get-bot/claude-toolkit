...
| File                   | Description                                                                                                                                       |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `package.json`         | 패키지 메타데이터, `bin: ssh-mcp → dist/index.js`, 전체 npm 스크립트. 런타임 의존성은 `@modelcontextprotocol/sdk`·`ssh2`·`zod` 셋뿐입니다.        |
...
- 런타임 의존성을 늘리지 마세요. 패키지는 네이티브 애드온 없이 순수 JS로 유지되며, `scripts/assert-no-native-addons.mjs`가 CI에서 이를 검증합니다.
