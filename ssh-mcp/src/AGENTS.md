...
| File         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `index.ts`   | bin 진입점. Node 20 이상 가드 후 argv 라우팅(`host` / `setup` / `doctor` / `install` / `--version`·`-v`·`version` / `help`·`--help`·`-h` / 그 외는 서버). `setup`은 `host add`의 **무경고 별칭**입니다. `run(argv)`, `MIN_NODE_MAJOR` export. 서브커맨드 모듈은 **동적 import**입니다.                                                                                                                                                                                                                                                                                                                                                      |
| `help.ts`    | 최상위 `help`/`--help`/`-h`. `USAGE`(명령어 목록)와 그 폭 상수 `USAGE_MAX_COLUMNS`·`USAGE_DESCRIPTION_COLUMN`, `runHelp(argv, deps)`, `HELP_TOKENS`(라우터가 **정적 import로 직접 읽는** 유일한 표), `VERSION_TOKENS`, `HELP_TOPICS`·`isHelpTopic`, `EXIT_OK`/`EXIT_USAGE`. import도 부수효과도 없는 문자열 모듈이라 서버 기동에 얹히지 않습니다. `help <command> ...`는 `<command> ... --help`로 **위임**하므로 플래그를 복사해 두지 않고, `help host add`는 `host add`에 닿습니다. 위임 함수는 주입받습니다 — `index.ts`를 import 할 수 없기 때문입니다. `HELP_TOPICS`는 라우터 분기의 거울이라 새 최상위 명령은 두 곳에 함께 추가합니다. |
...
최상위 파일들을 직접 겨냥하는 테스트: `tests/unit/audit.test.ts`, `tests/unit/redact.test.ts`, `tests/unit/errors.test.ts`, `tests/unit/toolsList.test.ts`, `tests/unit/helpCommand.test.ts`, `tests/integration/audit.test.ts`, `tests/integration/startupWarnings.test.ts`.

`index.ts`의 argv 라우팅 자체는 단위 테스트가 없습니다 — import 하는 순간 CLI가 실행되기 때문입니다(위의 import 금지 항목). 그래서 각 명령은 `runHost`/`runDoctor`/`runHelp`처럼 함수 단위로 검증합니다. 라우팅이 실제로 이어져 있는지를 빌드된 바이너리로 확인하는 곳은 둘로 나뉩니다: `help`/`--help`/`-h`와 `help <command>` 위임은 `tests/e2e/package.test.ts`가 스폰해서 보고, `--version`·`doctor` 같은 나머지 분기는 CI 워크플로(`.github/workflows/ssh-mcp-ci.yml`)의 번들 스모크 단계가 `node dist/index.js --version`, `doctor --patterns`를 직접 실행해서 봅니다. `host`·`install` 라우팅은 어느 쪽에도 없습니다.
