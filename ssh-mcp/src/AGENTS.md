...
`index.ts`의 argv 라우팅 자체는 단위 테스트가 없습니다 — import 하는 순간 CLI가 실행되기 때문입니다(위의 import 금지 항목). 라우터는 `commands.ts`의 표를 읽으므로 "어느 이름이 어느 모듈로 가는가"는 컴파일러가 확인하고, 각 명령은 `runHost`/`runDoctor`/`runHelp`처럼 함수 단위로 검증합니다. 표가 실제로 바이너리에 이어져 있는지를 확인하는 곳은 둘로 나뉩니다: `help`/`--help`/`-h`와 `help install`·`help host add` 위임은 `tests/e2e/package.test.ts`가 스폰해서 보고, `--version`·`doctor`는 CI 워크플로(`.github/workflows/ssh-mcp-ci.yml`)의 번들 스모크 단계가 `node dist/index.js --version`, `doctor --patterns`를 직접 실행해서 봅니다.
