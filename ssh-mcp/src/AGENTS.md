...
| `help.ts`    | 최상위 `help`/`--help`/`-h`. `USAGE`(명령어 목록), `runHelp(argv, deps)`, `HELP_TOPICS`. `help <command>`는 각 명령의 usage로 **위임**하므로 플래그를 복사해 두지 않습니다. 위임 함수는 주입받습니다 — `index.ts`를 import 할 수 없기 때문입니다.                                      |
...
`index.ts`의 argv 라우팅 자체는 단위 테스트가 없습니다 — import 하는 순간 CLI가 실행되기 때문입니다(위의 import 금지 항목). 그래서 각 명령은 `runHost`/`runDoctor`/`runHelp`처럼 함수 단위로 검증하고, 라우팅이 실제로 이어져 있는지는 빌드된 바이너리를 띄우는 `tests/e2e/`가 봅니다.
