...
```markdown
## 잘한 점
- API 응답이 느려서 Redis 캐시를 적용했다.
- 성능 문제를 직접 캡치해서 해결까지 가져간 게 좋았다.

## 개선점
- 캐시 무효화 전략 없이 넣어서 데이터 정합성 이슈가 터졌다.
- Cache-Aside vs Write-Through 비교 후 서비스에 맞는 전략을 확정하자.

## 배운 점
- Cache-Aside는 읽기 중심, Write-Through는 쓰기 정합성이 중요할 때.
- 캐시는 "붙이면 빨라지는 것"이 아니라 "무효화 전략까지 세트"다.
