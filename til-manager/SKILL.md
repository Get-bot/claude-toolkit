...
description: >
   ...
   기본 푸시 대상은 Notion과 TIL Git 레포이며, "퀄츠에도 올려줘", "블로그에도 올려줘", "지식베이스에도 올려줘"처럼
   명시적으로 요청하면 Quartz 지식베이스 레포에도 옵션으로 게시한다.
...
이 스킬은 6개의 레퍼런스 파일에 의존한다. **TIL 생성/푸시 작업을 시작하기 전에 해당 Phase에 필요한 파일을 `view` 도구로 반드시 읽어라.** SKILL.md 본문의 요약만으로는 템플릿 구조 누락·AI 문체 오염·Notion 속성 오류가 발생한다.
...
| `references/quartz-publish.md` | **Phase 3 — quartz 게시를 명시적으로 요청받았을 때만** | 영역 폴더 배치, 태그 kebab-case 변환, index.md 큐레이션, 커밋 규칙 |
...
[사용자 입력] → [TIL 생성] → [Notion DB 저장] → [사용자 리뷰/수정] → [Git 푸시]
     ↑                ↑                                                  │
  커밋 로그        raw 텍스트                                             ├→ TIL 레포 (기본)
  공부 내용        대화 내용 (Claude.ai)                                  └→ Quartz 지식베이스 (옵션, 명시 요청 시)
```
...
| **TIL 푸시** | "TIL 깃에 올려줘", "오늘 TIL 푸시해줘" | Notion에서 fetch → Markdown 변환 → TIL 레포에 Git push |
| **Quartz 게시 (옵션)** | "퀄츠에도 올려줘", "블로그에도 올려줘", "지식베이스에도 올려줘" | TIL 푸시 + Quartz 영역 폴더 배치 → index.md 한 줄 → push |
...
## Phase 3: Quartz 지식베이스 게시 (옵션)

> **🔴 시작 전 필수**: `view("references/quartz-publish.md")` — 영역 폴더 결정, 태그 변환, index.md 큐레이션, 커밋 규칙이 전부 거기에 있다.

**기본 푸시 대상은 Notion + TIL 레포다. 이 Phase는 사용자가 quartz를 명시적으로 요청했을 때만 실행한다.**
"TIL 깃에 올려줘"에는 quartz를 얹지 않고, 먼저 제안하지도 않는다.

| 트리거 | 동작 |
|--------|-----|
| "TIL 깃에 올려줘" | Phase 2만 (TIL 레포) |
| "퀄츠에도 / 블로그에도 / 지식베이스에도 올려줘", "둘 다 올려줘" | Phase 2 + Phase 3 |
| "퀄츠에만 올려줘" | Phase 3만 |

요약 흐름 — 상세는 전부 `references/quartz-publish.md` 가 정본이다:

1. `quartz_repo.md` 메모리에서 레포 정보 로드 (없으면 한 번만 질문 후 저장)
2. 레포의 작성 규칙 문서(`convention_doc`)가 있으면 읽는다 — **레포 규칙이 이 스킬보다 우선**
3. 영역 폴더 추정 → **근거와 함께 사용자 확인**
4. Phase 2에서 만든 Markdown 재사용. **본문은 그대로 두고 `tags` 만 소문자 kebab-case 영어로 변환**
5. 영역 `index.md` 큐레이션 한 줄 **초안을 보여주고 승인받는다**
6. TIL 파일 + index.md를 한 커밋으로 묶어 push (`docs(<area>): [TIL-YYMMDD] 주제명`)
...
| quartz 게시 요청인데 레포 정보 없음 | `references/quartz-publish.md`의 "정보 확인 순서"대로 한 번만 질문하고 메모리에 저장 |
| quartz push만 실패 | TIL 레포 푸시는 이미 끝났다는 것을 먼저 알리고, quartz만 재시도하거나 수동 절차를 안내 |
